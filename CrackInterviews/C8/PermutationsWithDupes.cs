...
public class PermutationsWithDupes
{
    ...
    private static void CalculateAux(Dictionary<string, int> dict, int remaining, IList<IList<string>> result,
        IList<string> tempResult)
    {
        if (remaining == 0)
        {
...
public class PermutationsWithDupesTests
{
    ...
    private static IEnumerable<TestCaseData> GetTestData()
    {
        ...
        yield return new TestCaseData(new[] {"1"}, new List<IList<string>> {new List<string> {"1"}});
        yield return new TestCaseData(new[] {"1", "2"},
            new List<IList<string>> {new List<string> {"1", "2"}, new List<string> {"2", "1"}});
        yield return new TestCaseData(new[] {"1", "2", "3"}, new List<IList<string>>
        {
        ...
        yield return new TestCaseData(new[] {"2", "2"}, new List<IList<string>> {new List<string> {"2", "2"}});
        yield return new TestCaseData(new[] {"1", "2", "2"},
            new List<IList<string>>
                {new List<string> {"1", "2", "2"}, new List<string> {"2", "1", "2"}, new List<string> {"2", "2", "1"}});
    }
}
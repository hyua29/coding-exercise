...
/// <summary>
///     Checkout this link for a general solution -
...
public class PermutationsWithoutDupes
{
    ...
    {
        if (buffer.Count == array.Length) results.Add(buffer.ToList());
...
public class PermutationsWithoutDupesTests
{
    ...
    public void CalculateTests(char[] array, IList<IList<char>> expectedResult)
    {
        ...
        for (var i = 0; i < result.Count; i++)
        {
            ...
            for (var j = 0; j < result[i].Count; j++) Assert.That(result[i][j], Is.EqualTo(expectedResult[i][j]));
        }
    }
    ...
    private static IEnumerable<TestCaseData> GetTestData()
    {
        ...
        yield return new TestCaseData(new[] {'1'}, new List<IList<char>> {new List<char> {'1'}});
        yield return new TestCaseData(new[] {'1', '2'},
            new List<IList<char>> {new List<char> {'1', '2'}, new List<char> {'2', '1'}});
        yield return new TestCaseData(new[] {'1', '2', '3'}, new List<IList<char>>
        {
            new List<char> {'1', '2', '3'},
            new List<char> {'1', '3', '2'},
            new List<char> {'2', '1', '3'},

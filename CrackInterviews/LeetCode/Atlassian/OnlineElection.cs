...
public class OnlineElection
{
    ...
    public OnlineElection(int[] persons, int[] times)
    {
        ...
        {
            var p = persons[i];
            if (!candidatesToVotes.TryAdd(p, 1)) candidatesToVotes[p]++;

            candidatesToVotes.TryGetValue(leader, out var oldLeaderVotes);

            if (candidatesToVotes[p] >= oldLeaderVotes) leader = p;
    ...
    public class OnlineElectionTests
    {
        ...
        public void OnlineElectionTest(int[] persons, int[] times, int[] query, int[] expectedResults)
        {
            ...
            for (var i = 0; i < query.Length; i++) Assert.That(e.Q(query[i]), Is.EqualTo(expectedResults[i]));
        }

        private static IEnumerable<TestCaseData> GetTestCaseDate()
        {
            yield return new TestCaseData(new[] {0, 1, 1, 0, 0, 1, 0}, new[] {0, 5, 10, 15, 20, 25, 30},
                new[] {3, 12, 25, 15, 24, 8}, new[] {0, 1, 1, 0, 0, 1});

            yield return new TestCaseData(new[] {0, 1, 0, 1, 1}, new[] {24, 29, 31, 76, 81},
                new[] {28, 24, 29, 77, 30, 25, 76, 75, 81, 80}, new[] {0, 0, 1, 1, 1, 0, 1, 0, 1, 1});
        }
    }
}
...
/// <summary>
/// See this for details: https://leetcode.com/problems/unique-paths-ii/solutions/1180249/easy-solutions-w-explanation-comments-optimization-from-brute-force-approach/?orderBy=most_votes
/// </summary>
public class UniquePathsII
{
    public int UniquePathsWithObstacles(int[][] obstacleGrid)
    {
        ...
        int[][] buffer = new int[obstacleGrid.Length][];
        for (int i = 0; i < buffer.Length; i++)
        {
            buffer[i] = new int[obstacleGrid[0].Length];
        }
    ...
    private int ExploreOtherRoutes((int X, int Y) currentPosition, int[][] obstacleGrid, int[][] buffer)
    {
        ...
        if (currentPosition.X == obstacleGrid[0].Length - 1 && currentPosition.Y == obstacleGrid.Length - 1)
        {
            return 1;
        }
...
public class UniquePathsIITest
{
    ...
    {
        int[][] obstacleGrid = new int[][] {new int[] {0}};
        UniquePathsII uniquePathsII = new UniquePathsII();
        int uniquePaths = uniquePathsII.UniquePathsWithObstacles(obstacleGrid);
    ...
    {
        int[][] obstacleGrid = new int[][] {new int[] {1}};
        UniquePathsII uniquePathsII = new UniquePathsII();
        int uniquePaths = uniquePathsII.UniquePathsWithObstacles(obstacleGrid);

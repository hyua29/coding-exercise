...
public class ListOfDepths
{
    public static IList<SinglyLinkedListNode> GetListOfDepths(BinaryTreeNode<int> root)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (currentTreeNode.LeftNode != null) queue.Enqueue(currentTreeNode.LeftNode);

            if (currentTreeNode.RightNode != null) queue.Enqueue(currentTreeNode.RightNode);
        }
...
public class ListOfDepthsTests
{
    ...
    public void ListOfDepths_Test(BinaryTreeNode<int> root, IList<List<int>> expectedResult)
    {
        ...
        for (var i = 0; i < expectedResult.Count; i++)
        {
            ...
            for (var j = 0; j < er.Count; j++) Assert.That(ar[j].Data, Is.EqualTo(er[j]));
        }
    }
    ...
    private static IEnumerable<TestCaseData> GetTestData()
    {
        ...
        var expectedResults = new List<List<int>> {new() {1}, new() {3, 2}, new() {5, 4}};
        ...
        var expectedResults2 = new List<List<int>>
            {new() {1}, new() {3, 2}, new() {5, 4}, new() {9, 8, 7, 6}};

...
public class QuickSort
{
    ...
    private static void Sort(int start, int end, int[] array)
    {
        if (start < end)
        {
            var pivotIndex = Partition(start, end, array);
    ...
    public void QuickSortTest(int[] arrayToSort)
    {
        ...
        var result = Calculate1(arrayToSort);
        Assert.That(result.Length, Is.EqualTo(expectedResult.Length));
        for (var i = 0; i < arrayToSort.Length; i++) Assert.That(result[i], Is.EqualTo(expectedResult[i]));
    }

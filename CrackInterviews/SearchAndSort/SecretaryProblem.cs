...
public class SecretaryProblem
{
    public static double Calculate(int choiceCount)
    {
        ...
        var optimumSampleSize = -1;
        var optimumProbability = double.MinValue;
        for (var sampleSize = 0; sampleSize < choiceCount; sampleSize++)
        {
            ...
            for (var i = 0; i < permutations.Count; i++)
            {
                ...
                if (sampleSize > 0)
                    for (var j = 0; j < sampleSize; j++)
                        if (sampleMax < permutations[i][j])
                            sampleMax = permutations[i][j];

                for (var j = sampleSize; j < choiceCount; j++)
            ...
            var currentOptimumProbability = (double) picks.Count(x => x == choiceCount) / picks.Length;
...
public class SecretaryProblemTests
{
    ...
    public void GetPermutationsTests(int choiceCount)
    {
        ...
        else if (choiceCount > 0)
        {
            ...
            for (var i = 1; i < choiceCount + 1; i++) expectedLength *= i;
